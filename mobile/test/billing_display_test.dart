...
void main() {
  ...
  group('a transaction is only finished once the backend grants it', () {
    ...
    test('a cancelled transaction never reaches the callable', () async {
      final store = _FakeStore();
      final sent = <Map<String, dynamic>>[];
      final outcomes = <PurchaseOutcome>[];
      final service = _serviceWith(store, onVerify: sent.add)..listen();
      service.results.listen((r) => outcomes.add(r.outcome));

      await deliver(store, delivered(PurchaseStatus.canceled));

      // Nothing was bought, so nothing may be blamed on the backend.
      expect(sent, isEmpty);
      expect(outcomes, [PurchaseOutcome.cancelled]);
      await service.dispose();
    });
